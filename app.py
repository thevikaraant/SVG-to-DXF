...
def to_dxf_file():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        return send_file(out_dxf, mimetype="application/dxf",
                         as_attachment=True, download_name="output.dxf")

@app.route("/to-dxf-url", methods=["POST"])
def to_dxf_url():
    data = request.get_json(silent=True)
    if not data or "url" not in data:
        return jsonify(error="JSON with 'url' required"), 400
    url = data["url"]
    resp = requests.get(url, stream=True)
    if resp.status_code != 200:
        return jsonify(error=f"Fetch failed ({resp.status_code})"), 400
    with tempfile.TemporaryDirectory() as tmp:
        in_svg  = os.path.join(tmp, "in.svg")
        out_eps = os.path.join(tmp, "out.eps")
        out_dxf = os.path.join(tmp, "out.dxf")
...
@app.route("/")
def index():
    return "SVG→DXF Converter is live (endpoints: /to-dxf, /to-dxf-url)"
